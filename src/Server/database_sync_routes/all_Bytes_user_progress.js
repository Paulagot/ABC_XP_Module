...
import pool from '../config_db.js'; // Use your DB connection module
    ...
    for (const courseId of zenlerIds) {
      ...
      for (const uid in courseData) {
        ...
        pool.query('SELECT user_id FROM users WHERE email = ?', [email], (err, userResults) => {
          ...
          pool.query('SELECT bite_id, category_id, subcategory_id, points FROM bites WHERE zenler_id = ?', [courseId], (err, biteResults) => {
